#!/usr/bin/env python3
"""Verify provider prompt-caching visibility through an OpenAI-compatible chat endpoint.

This script can:
- send multiple requests in a row (default: 5)
- test either a shared-prefix prompt or an identical full prompt
- optionally run both variants
...
Usage examples:
    ...
    uv run python devtools/check_prompt_cache.py

    uv run python devtools/check_prompt_cache.py --variant both --requests 5

    uv run python devtools/check_prompt_cache.py \
      --api-base https://cody.ib-inet.com \
      --api-key "$API_KEY" \
      --model gpt-5-nano \
      --variant identical-full
"""
...
def parse_args() -> CliArgs:
    parser = argparse.ArgumentParser(
        description=(
            "Send multiple nearly identical requests to an OpenAI-compatible chat "
            "endpoint and print usage fields that may indicate provider prompt caching."
        )
    )
    ...
    _ = parser.add_argument(
        "--endpoint",
        default=os.environ.get("CHAT_ENDPOINT", "/v1/chat/completions"),
        help="Chat completions endpoint path (default: /v1/chat/completions)",
    )
    _ = parser.add_argument(
        "--repeat-sentences",
        type=int,
    ...
    _ = parser.add_argument(
        "--max-tokens",
        type=int,
        default=int(os.environ.get("MAX_TOKENS", "32")),
        help="max_tokens sent with each request",
    )
    _ = parser.add_argument(
        "--show-full-response",
        action="store_true",
...
def build_user_suffix(index: int, variant: Variant) -> str:
    if variant == "identical-full":
        return "Reply with exactly: cache-check"
    return f"Reply with exactly: request-{index}"


def build_payload(model: str, shared_prefix: str, suffix: str, max_tokens: int) -> JSONDict:
    return {
        "model": model,
        "stream": False,
        "messages": [
...
def extract_usage_stats(response_data: JSONDict) -> UsageStats:
    ...
    return UsageStats(
        prompt_tokens=find_first_key(usage, "prompt_tokens"),
        completion_tokens=find_first_key(usage, "completion_tokens"),
...
def print_variant_interpretation(variant: Variant, results: list[RequestResult]) -> None:
    ...
    for result in results:
        line = (
            f"  {result.label}: prompt={result.usage.prompt_tokens}, "
...
def run_variant(
    *,
    ...
    if variant == "identical-full":
        ...
        print("  Sending the exact same full prompt body for every request.")
    else:
        print("Variant: shared-prefix")
        print("  Keeping the large shared prefix constant while varying only the tail.")
    print(f"  Request count: {request_count}")

    results: list[RequestResult] = []
    for index in range(1, request_count + 1):
        suffix = build_user_suffix(index, variant)
        response_data = post_json(
            url,
            api_key,
            build_payload(model, shared_prefix, suffix, max_tokens),
        )
...
def main() -> int:
    ...
    print("Prompt-cache verification (chat completions)")
    print(f"  URL: {url}")
    print(f"  Model: {model}")
